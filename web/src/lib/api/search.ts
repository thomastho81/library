...
export type InventorySummaryParams = Omit<
  SearchParams,
...
function normalizeInventorySummary(data: unknown): InventorySummaryResponse {
  const raw = data as Record<string, unknown>;
  return {
    totalBooks: typeof raw.totalBooks === "number" ? raw.totalBooks : 0,
    totalAvailableCopies:
      typeof raw.totalAvailableCopies === "number" ? raw.totalAvailableCopies : 0,
    booksWithReservedCopies:
      typeof raw.booksWithReservedCopies === "number" ? raw.booksWithReservedCopies : 0,
  };
}

/** Resumo agregado do inventário (GET /api/search/books/summary/inventory). */
export async function fetchInventorySummary(
  ...
  const url = `${BASE}/api/search/books/summary/inventory${buildQuery(params)}`;
  log("GET", url);
  ...
  try {
    ...
    const data = await res.json();
    const normalized = normalizeInventorySummary(data);
    log("ok, summary:", normalized);
    return normalized;
  } catch (err) {
    logError("fetchInventorySummary falhou", err);
    throw err;
  }
}
